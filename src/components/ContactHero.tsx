  ...
  useEffect(() => {
    ...
    mm.add("(max-width: 767px)", () => {
      const ctx = gsap.context(() => {
        gsap.fromTo(".contact-bg", { opacity: 0, scale: 1.05 }, { opacity: 0.8, scale: 1, duration: 1.5, ease: "power3.out" });
        gsap.fromTo(".char", { y: 30, opacity: 0 }, { y: 0, opacity: 1, stagger: 0.025, duration: 0.8, ease: "power3.out", delay: 0.3 });
        gsap.fromTo(".contact-sub", { y: 20, opacity: 0 }, { y: 0, opacity: 1, duration: 1, delay: 0.6, ease: "power3.out" });
      }, containerRef);
      return () => ctx.revert();
    });
    ...
    mm.add("(min-width: 768px)", () => {
      const ctx = gsap.context(() => {
        gsap.fromTo(".contact-bg", { opacity: 0, scale: 1.1 }, { opacity: 0.8, scale: 1, duration: 2.5, ease: "power2.out" });
        gsap.fromTo(".char", { y: 100, opacity: 0, rotateX: -90 }, { y: 0, opacity: 1, rotateX: 0, stagger: 0.05, duration: 1.5, ease: "expo.out", delay: 0.5 });
        gsap.fromTo(".contact-sub", { y: 30, opacity: 0 }, { y: 0, opacity: 1, duration: 1.5, delay: 1.2, ease: "power3.out" });
      }, containerRef);
      return () => ctx.revert();
    });
  ...
  return (
    <section ref={containerRef} className="relative h-screen w-full flex flex-col items-center justify-center overflow-hidden bg-foreground">
      ...
      <div className="relative z-10 text-center px-6">
        ...
        <h1 className="text-2xl sm:text-4xl lg:text-[6.5vw] font-serif text-background tracking-tighter leading-tight lg:leading-none perspective-1000 flex flex-nowrap justify-center">
        ...
        <div className="contact-sub max-w-2xl mx-auto mt-6 md:mt-16">
          <p className="text-xl md:text-3xl font-serif text-background/60 italic leading-relaxed">
            "Your journey continues with a single conversation."
          </p>
        </div>
      </div>
      ...
      <div className="absolute bottom-12 left-1/2 -translate-x-1/2 flex flex-col items-center gap-6 opacity-30">
        ...
        <span className="text-[10px] uppercase tracking-[0.6em] font-black text-background">Inquire</span>
      </div>
    </section>
  );
}
